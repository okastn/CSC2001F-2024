...
public class Grid {
	...
	private int [][] update;//grid for next time step
	...
	public Grid(int w, int h) {
		...
		update=new int[this.rows][this.columns];
	...
	//for the next timestep - copy update into grid
	...
	boolean update() {
		boolean change = false; // Initialize the change variable
		SandpileUpdateTask task = new SandpileUpdateTask(grid, update, 1, rows - 1);
        pool.invoke(task);

        // Check for changes and copy update to grid
